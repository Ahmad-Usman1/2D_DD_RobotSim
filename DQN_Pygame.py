...
class Environment:
    ...
    def reset(self):
        ...
        while True:
            ...
            dist = math.sqrt(
                (self.goal_x - self.agent_x) ** 2 + (self.goal_y - self.agent_y) ** 2
            )
...
class Visualizer:
    ...
    def render(self, episode, reward, epsilon):
        ...
        # Draw goal
        pygame.draw.circle(
            self.screen, (0, 255, 0), (int(self.env.goal_x), int(self.env.goal_y)), 20
        )
...
def test_agent(agent, num_episodes=10, visualize=True):
    ...
    for episode in range(num_episodes):
        ...
        print(
            f"Test Episode {episode + 1}: Reward={episode_reward:.2f}, Steps={step + 1}"
        )
